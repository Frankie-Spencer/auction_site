    ...
    init: function(field_id, field_name, is_stacked, admin_static_prefix) {
        ...
        for (var i=0; i<ps.length; i++) {
            if (ps[i].className.indexOf("info") != -1) {
                ...
                // boxes or wrregisterapped off on the side to the right of the add
                // button:
        ...
        filter_p.registerappendChild(document.createTextNode(' '));

        var filter_input = quickElement('input', filter_p, '', 'type', 'text', 'placeholder', gettext("Filter"));
        filter_input.id = field_id + '_input';

        selector_available.registerappendChild(from_box);
